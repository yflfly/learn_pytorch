...
例如data = torch.Tensor(2,3)是一个2*3的张量，类型为FloatTensor;
...
还可以使用type()函数，data为Tensor数据类型，data.type()为给出data的类型，
...
（4）Variable变量转换成普通的Tensor，其实可以理解Variable为一个Wrapper，里头的data就是Tensor.
 如果Var是Variable变量，使用Var.data获得Tensor变量
