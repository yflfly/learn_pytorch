...
'''
print('---------------------ll-------------------')
sorted, indices = torch.sort(x, descending=True)  # 按照从大到小排序(即反序)
print(x)
print(sorted)
print(indices)
...
'''
输出结果如下所示：
tensor([[-0.1289, -0.8730, -0.2705,  0.0732],
        [ 1.4079, -1.2794, -1.2175,  0.8988],
        [ 0.2018,  0.3004,  0.5547, -0.9283]])
tensor([[ 0.0732, -0.1289, -0.2705, -0.8730],
        [ 1.4079,  0.8988, -1.2175, -1.2794],
        [ 0.5547,  0.3004,  0.2018, -0.9283]])
tensor([[3, 0, 2, 1],
        [0, 3, 2, 1],
        [2, 1, 0, 3]])
'''
...
'''
输出结果如下所示：
tensor([[ 1.9161,  0.9730,  0.5625, -1.1244],
        [-0.6170,  0.0555, -0.6609,  0.8201],
        [-1.4844,  1.1053, -1.1549,  0.6375]])
tensor([[-1.4844,  0.0555, -1.1549, -1.1244],
        [-0.6170,  0.9730, -0.6609,  0.6375],
        [ 1.9161,  1.1053,  0.5625,  0.8201]])
tensor([[2, 1, 2, 0],
        [1, 0, 1, 2],
        [0, 2, 0, 1]])
'''
...
'''
输出结果如下所示：
tensor([[ 0.0082,  0.1723,  0.4803, -0.4050],
        [ 3.0722,  1.8905, -0.7182,  0.4499],
        [ 0.3160, -0.0740, -0.6020, -1.0778]])
tensor([[ 3.0722,  1.8905,  0.4803,  0.4499],
        [ 0.3160,  0.1723, -0.6020, -0.4050],
        [ 0.0082, -0.0740, -0.7182, -1.0778]])
tensor([[1, 1, 0, 1],
        [2, 0, 2, 0],
        [0, 2, 1, 2]])
'''
